...
docker rm -f qwen3-0.6b-mindie 2>/dev/null || true

docker run -itd --privileged \
  --name=qwen3-0.6b-mindie \
  --net=host \
  --shm-size=32g \
  --device=/dev/davinci1 \
  ...
  -v "${SCRIPT_DIR}/qwen3-0.6b-mindie-card1-config.json:/tmp/mindie-config.json:ro" \
  -v "${SCRIPT_DIR}/qwen3_template_overrides/Qwen3-0.6B/tokenizer_config.json:/opt/models/MindSDK/Qwen3-0.6B/tokenizer_config.json:ro" \
