...
docker rm -f qwen3-14b-mindie >/dev/null 2>&1 || true

docker run -itd --privileged \
  --name=qwen3-14b-mindie \
  --net=host \
  --shm-size=64g \
  --device=/dev/davinci2 \
  ...
  -v "${SCRIPT_DIR}/qwen3-14b-mindie-card2-config.json:/tmp/mindie-config.json:ro" \
  -v "${SCRIPT_DIR}/qwen3_template_overrides/Qwen3-14B/tokenizer_config.json:/opt/models/MindSDK/Qwen3-14B/tokenizer_config.json:ro" \
