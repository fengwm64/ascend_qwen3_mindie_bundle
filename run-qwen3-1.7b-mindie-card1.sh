...
docker rm -f qwen3-1.7b-mindie 2>/dev/null || true

docker run -itd --privileged \
  --name=qwen3-1.7b-mindie \
  --net=host \
  --shm-size=32g \
  --device=/dev/davinci1 \
  ...
  -v "${SCRIPT_DIR}/qwen3-1.7b-mindie-card1-config.json:/tmp/mindie-config.json:ro" \
  -v "${SCRIPT_DIR}/qwen3_template_overrides/Qwen3-1.7B/tokenizer_config.json:/opt/models/MindSDK/Qwen3-1.7B/tokenizer_config.json:ro" \
